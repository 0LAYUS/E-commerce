...
import { createProduct, updateProduct, deleteProduct, getProductOptions, getProductVariants, getProductImages, toggleProductActive, hasSales } from "@/features/products/actions/productActions"
    ...
    <div className="flex flex-col h-screen px-4 py-4 overflow-hidden">
      <div className="flex justify-between items-center mb-4 shrink-0">
        ...
        <div className="flex gap-3">
          <Link href="/admin/products/archived" className="border border-input hover:bg-accent px-4 py-2.5 rounded-lg font-semibold text-sm flex items-center gap-2 transition">
            ...
            <div key={p.id} className={`bg-card rounded-xl shadow-sm border overflow-hidden flex flex-col hover:shadow-md transition h-full ${(p as { active?: boolean }).active === false ? 'opacity-50' : ''}`}>
              ...
              <div className="p-3 flex flex-col flex-grow">
                ...
                <div className="flex gap-2 mt-auto">
                  ...
                  <button onClick={() => openDeleteConfirm(p.id, p.name)} className="w-12 flex justify-center items-center border border-input text-destructive rounded-lg hover:bg-destructive/10 hover:border-destructive/20 transition">
          ...
          <div className="bg-card rounded-2xl shadow-xl w-full max-w-3xl my-8 border" onClick={(e) => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              ...
              <div>
                ...
                <input name="name" defaultValue={editingProduct?.name} required className="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              {editingProduct && (
              ...
              <div>
                ...
                <textarea name="description" defaultValue={editingProduct?.description} required rows={3} className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-semibold text-card-foreground mb-1.5">Precio base</label>
                  <input type="number" name="price" defaultValue={editingProduct?.price} required className="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring" />
                </div>
                <div>
                  <label className="block text-sm font-semibold text-card-foreground mb-1.5">Categoría</label>
                  <select name="category_id" defaultValue={editingProduct?.category_id || ""} required className="w-full h-10 rounded-md border border-input bg-background px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring">
              ...
              <div>
                <label className="block text-sm font-semibold text-card-foreground mb-1.5">{hasVariants ? "Stock total" : "Stock"}</label>
                <input type="number" name="stock" value={hasVariants ? totalVariantStock : (editingProduct ? editingProduct.stock : newProductStock)} disabled={hasVariants} onChange={hasVariants ? undefined : (e) => { const val = e.target.value; if (val === "") { if (editingProduct) setEditingProduct({ ...editingProduct, stock: 0 }); else setNewProductStock(0); } else { const num = parseInt(val, 10); if (!isNaN(num)) { if (editingProduct) setEditingProduct({ ...editingProduct, stock: num }); else setNewProductStock(num); } } }} required={!hasVariants} readOnly={hasVariants} className={`w-full h-10 rounded-md border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-ring ${hasVariants ? "bg-muted text-muted-foreground cursor-not-allowed" : "bg-background"}`} />
              ...
              {loadingVariants ? (
                ...
                <ProductVariantsEditor initialOptions={variantOptions} initialVariants={variantStocks.map(v => ({ id: v.id, sku_code: v.sku_code, stock: v.stock, active: v.active, price_override: v.price_override, option_values: [] }))} hasVariants={hasVariants} onHasVariantsChange={handleHasVariantsChange} />
              )}
      ...
      <ConfirmDialog open={deleteConfirmOpen} onClose={() => { setDeleteConfirmOpen(false); setDeleteTarget(null) }} onConfirm={handleDelete} title="¿Eliminar producto?" description={deleteTarget ? `¿Eliminar "${deleteTarget.name}"?` : ""} confirmText="Eliminar" cancelText="Cancelar" destructive />
      <ConfirmDialog open={archiveConfirmOpen} onClose={() => { setArchiveConfirmOpen(false); setDeleteTarget(null) }} onConfirm={() => { handleArchiveConfirm(); setArchiveConfirmOpen(false) }} title="Archivar producto" description={alertConfig.description} confirmText="Archivar" cancelText="Cancelar" destructive />
